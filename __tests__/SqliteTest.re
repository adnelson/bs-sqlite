...
let () = {
  describe("Basic functionality", () => {
    ...
    test(
      ...
      runner(db, 2, "SELECT 1+1 AS result", s =>
        Sqlite.Statement.get(s, [||])
      ),
    );

    test(
      "Should interpolate un-named parameters",
      runner(db, 3, "SELECT ? + ? AS result", s =>
        Sqlite.Statement.get(s, [|1, 2|])
      ),
    );
  ...
  describe("File based database", () => {
    ...
    test("insert and retrieve record from file database", () => {
      let db =
        Sqlite.Connection.make(~path=db_path, ~fileMustExist=true, ());
